...
def ground_segmentation(data):
    ...
    # segmengted_cloud = data[np.where(np.abs(best_d) >= d_threshold)[0], :]  
    # ground_idx = np.where(np.abs(best_d) < d_threshold)[0]
    # non_ground_idx = np.where(np.abs(best_d) >= d_threshold)[0]    
...
def clustering(data):
    ...
    # 屏蔽开始    
    now = time.time()    
    my_clusters_index = my_dbscan(data, r=0.5, minPts=10)
